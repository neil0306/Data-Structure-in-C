...
将链表里的节点逆置:
    ...
    只需要完成一个demo, 不考虑众多 corner case.
```

思路
```txt
假如我们要对 " 1 2 3 4" 这样的单链表进行逆置, 过程如下:

辅助变量:
- 定义一个 p 指针, 指向"下一个要交换的节点".
- 定义一个 n 指针, 只想"下一个要交换的节点" 的 "下一个节点".
```

过程图解:
![](单链表应用_逆置单链表_images/单链表应用_逆置单链表_过程图解.png)
