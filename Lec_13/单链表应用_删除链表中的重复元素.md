# 单链表应用 删除链表中的重复元素
```txt
只考虑简单的情况 -- 链表节点的数据都是 0-9 中的一个数字
现在希望去除链表中重复出现的元素
    如: 1 2 1 4 5 4 => 1 2 4 5
```
...
解决思路:
```txt
初始化辅助数组为 0, 由于链表节点的数据都是 0-9 中的一个数字
, 所以辅助数组的下标为 0-9
    - 从头开始遍历链表, 并根据链表节点中存放的数值检查辅助数组中对应下标的值
        - 如果为 0, 则将辅助数组中对应下标的值置为 1
        - 如果为 1, 则删除该节点
```
