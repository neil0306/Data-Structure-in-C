...
{
    struct node * p = linklist_head->next;       // 下一个要操作的节点
    struct node * n;                    // 下一个要操作的节点 的 再下一个节点

    // 初始化
    linklist_head->next = NULL;
    tail = linklist_head;        // 逆置后, 尾指针就是现在的头指针指向的位置
    ...
    head = linklist_head;       
    /*
        head在这个程序demo里是全局变量, 而入参时, 本质上是"按值传递", 
            即 linklist_head 属于局部变量, 它拷贝了一份 head 的值而已.

        如果少了这行代码, 主程序在使用 head 的时候只会打印 "原本全局变量head所指向的节点",
            并且由于这里通过指针进行链表逆置操作, 原本的第一个已经变成最后一个节点,
            它的next指针指向NULL, 此时的打印效果是只有一个节点的输出.
    */
}