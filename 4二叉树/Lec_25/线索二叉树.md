...
## 遍历中序线索化二叉树
此时不能直接复用普通二叉树的中序遍历函数了, 因为线索化二叉树的左右子树指针域已经被利用起来了, 没有指向真正子树的指针不再指向NULL, 而是指向前趋点/后继点.
...
step1: 找到二叉树最左下的节点
    此时需要通过循环来找到它
        while(lflag == 0){      // lfag 为 0, 表示它的左子树存在, 于是我们一直往左走
            tree = tree->ltree;
        }

        在这棵二叉树中, D节点就是没有左子树的, 并且是最左侧节点, 故D就是第一个节点
...
step 2: 由于已经找到的是 "最左下节点", 此时这个节点有两种情况(这棵树里的D或者F)
    ...
    case2: 这个节点有右子树 ==> 把这棵右子树进行中序遍历, 中序遍历得到的第一个节点就是后继节点(右子树的 "最左下节点" ==> 又回到了step1)
        当前节点有右子树, 所以当前节点并没有直接连接着它的后继节点, 所以这里需要更新一下遍历的节点位置, 对于这个case, 输出后继点的任务是交给 step1 来完成的.
        这课树的 F节点 就是这种情况.
```
